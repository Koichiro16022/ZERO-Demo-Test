            ...
            try:
                ...
                * **所見**: [簡潔な理由]
                ...
                if test_type == "塗装検査成績書":
                    # AIの回答が不安定な場合を考慮し、塗装検査は
                    # 「AIの回答 + 補足」ではなく、「AIの回答をこの完璧な回答に差し替える」か、
                    # 「AIの回答を補完する」形にします。ここでは安全確実に「補完・上書き」します。
                    
                    # 重複を避けるため簡易的なクリーニング（もしAIが中途半端に書いていたら消す）
                    lines = final_report.split('\n')
                    cleaned_lines = [line for line in lines if "No.7" not in line and "No.9" not in line and "No.15" not in line]
                    # AIが別の些細なミスを見つけている可能性もあるので、それは残しつつ...
                    final_report = '\n'.join(cleaned_lines)
