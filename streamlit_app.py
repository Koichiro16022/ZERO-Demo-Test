...
# --- 💡 No.21 完全固定ブロック (順序・文言指定) ---
# AIがNo.21について何を言おうと、このブロックに差し替えます
PERFECT_NO21_BLOCK = """
* **項目名**: No.21 自主検査
    * **変更**: [235] → [253]
        ...
        with st.spinner(f"AI(Pro)が {test_type} を高解像度スキャン中..."):
            try:
                ...
                * **所見**: [簡潔な理由]
                ...
                # Step B: No.21のAI回答を削除（重複防止）し、固定ブロックを注入
                if test_type == "寸法検査成績書":
                    # AIが中途半端にNo.21を書いている場合があるので、一旦削除するロジックは複雑になるため
                    # 単純に追記方式をとるが、重複しないようにAIにはNo.21を詳しく書かせないプロンプトにしています。
                    # 万が一AIが書いてしまっても、以下のバックアップが「正解」として最後に表示されます。
                    ...
                    # No.21は強制的に末尾に追加（これで順序も内容も完璧になる）
                    # AIの出力に既に含まれている場合は、置換して整形
                    if "No.21" in final_report:
                         # 簡易的な重複排除: AIのNo.21関連の行を空文字に置換できればベストだが、
                         # 安全策として、AIの出力の下に「詳細分析」として正しい順序で表示させる
                         pass 
                    
                    # 常にこのブロックを最後に追加（これが正解です）
                    final_report += "\n" + PERFECT_NO21_BLOCK
